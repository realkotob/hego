...
func _ready():
	if not label or not x_spin_box or not y_spin_box:
		push_error("Int2ParmUI: One or more nodes are null. Check scene structure: Label=%s, XSpinBox=%s, YSpinBox=%s" % [label, x_spin_box, y_spin_box])
		return
...
func setup(_param: Dictionary):
	initializing = true
	if not label:
		push_error("Int2ParmUI: Label node is null. Cannot set label text.")
		return
	if not x_spin_box or not y_spin_box:
		push_error("Int2ParmUI: SpinBox nodes are null. Cannot set values.")
		return
	...
	x_spin_box.value = param.get("values", [0, 0])[0]
	y_spin_box.value = param.get("values", [0, 0])[1]
...
func _on_x_value_changed(value: float):
	...
	value_changed.emit(param.get("name", ""), Vector2i(param["values"][0], param["values"][1]))
